...
Java_lameencode_LameEncode_init(JNIEnv *env, jclass jclass1, jint sampleRate,
                                     jint channelCount,
...
Java_lameencode_LameEncode_encoder(JNIEnv *env, jclass jclass1, jshortArray pcm_buffer,
    ...
    if (lame_get_num_channels(gfp) == 2) {
        encode_result = lame_encode_buffer_interleaved(gfp, pcm_buf, sample_num / 2,
                                                       reinterpret_cast<unsigned char *>(mp3_buf),
                                                       mp3_buff_len);
    } else {
        encode_result = lame_encode_buffer(gfp, pcm_buf, pcm_buf, sample_num,
                                           reinterpret_cast<unsigned char *>(mp3_buf),
                                           mp3_buff_len);
    }
    //释放资源
...
//关闭MP3编码buffer
extern "C" JNIEXPORT jint JNICALL
Java_lameencode_LameEncode_flush(JNIEnv *env, jclass jclass1, jbyteArray mp3_buffer) {
    //获取MP3数组长度
    const jsize mp3_buff_len = env->GetArrayLength(mp3_buffer);
    //获取buffer指针
    jbyte *mp3_buf = env->GetByteArrayElements(mp3_buffer, JNI_FALSE);
    //刷新编码器缓冲，获取残留在编码器缓冲里的数据
    int flush_result = lame_encode_flush(gfp, reinterpret_cast<unsigned char *>(mp3_buf), mp3_buff_len);
    env->ReleaseByteArrayElements(mp3_buffer, mp3_buf, 0);
    return flush_result;
}
...
//释放编码器
extern "C" JNIEXPORT void JNICALL
Java_lameencode_LameEncode_close(JNIEnv *env, jclass type) {
    lame_close(gfp);
}
